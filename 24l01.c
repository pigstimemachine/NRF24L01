...
#include <string.h>
#include "lib.h"		
const u8 TX_ADDRESS[TX_ADR_WIDTH]={0x34,0x43,0x10,0x12,0x00}; //发送地址
const u8 RX_ADDRESS[RX_ADR_WIDTH]={0x34,0x43,0x10,0x12,0x01}; //发送地址
ArrayBufType TxBufType,RxBufType;
...
void NRF24L01_RX_Mode(void)
{
	...
	NRF24L01_Write_Reg(WRITE_REG+RF_SETUP,0x26);//设置TX发射参数,0db增益,2Mbps,低噪声增益开启   
...
{														 
	...
	NRF24L01_Write_Buf(WRITE_REG+RX_ADDR_P0,(u8*)Rf24l01Para.RX_ADDRESS,RX_ADR_WIDTH); //设置TX节点地址,主要为了使能ACK	  

//	NRF24L01_Write_Reg(WRITE_REG+CONFIG,0x30);
	NRF24L01_Write_Reg(WRITE_REG+EN_AA,0x01);     //使能通道0的自动应答    
	NRF24L01_Write_Reg(WRITE_REG+EN_RXADDR,0x01); //使能通道0的接收地址  
	...
	NRF24L01_Write_Reg(WRITE_REG+RF_SETUP,0x26);//0x07);  //设置TX发射参数,0db增益,2Mbps,低噪声增益开启   
...
void TASK_NRF24L01_AutoSend(void)
{
	 FrameBufType Tmpbuf;
   if(ArrayGetFrame(&TxBufType,&Tmpbuf)==ABUF_NO_ERR)
	 {
		  Rf24l01Para.TX_ADDRESS[4]=Tmpbuf.MSGID&0xff;
	    Rf24l01Para.Txflag= NRF24L01_TxPacket(Tmpbuf.FrameData,6000);
	 }	 	 
}	
...
{
	 memcpy(Rf24l01Para.TX_ADDRESS,Txaddr,TX_ADR_WIDTH);
}
void NRF24L01_ModifyRxAddr(u8* Rxaddr)
{
   memcpy(Rf24l01Para.RX_ADDRESS,Rxaddr,RX_ADR_WIDTH);
}	
